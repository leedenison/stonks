module github.com/leedenison/stonks

go 1.27.1

// Every Go-based build tool is a tool directive, invoked as `go tool <name>`, so
// its version is pinned here and in go.sum like any other dependency and changes
// only in a reviewed diff. Tools that are not Go programs are pinned by an image
// tag or a lockfile instead. buf runs its protoc plugins as subprocesses, so a
// generation template names each one as `local: ["go", "tool", "protoc-gen-go"]`
// rather than expecting it on PATH.
tool (
