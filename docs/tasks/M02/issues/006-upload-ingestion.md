...
- Validation of each row. An invalid row is rejected on its own and recorded as an item
  of the run; the remaining rows are ingested.
- Replacement of every transaction for the claimed broker and period, so a re-upload
  is idempotent and an empty range at a boundary is a deletion.
